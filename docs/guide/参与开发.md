...
```bash
embykeeper config.toml -D all@test_bot 2>&1 | tee log.json
```

Docker 版本需要运行:

```bash
docker run -v $(pwd)/embykeeper:/app --rm -it --net=host embykeeper/embykeeper -D all@test_bot 2>&1 | tee log.json
```
...
项目统一使用 `uv` + `pyproject.toml` 管理 Python 版本和依赖, 开始前请先安装 [uv](https://docs.astral.sh/uv/)。
...
│   │   └── ...
│   └── index.md                # 文档首页布局文件
├── pyproject.toml              # Python 项目定义与依赖文件.
