...
`time` / `client` / `device` / `device_id` / `client_version` / `useragent` 可以作为全局默认设置, 被 `emby.account` 中的同名设置覆盖.
...
| `time` | `list` / `int` | 全局模拟观看时长, `emby.account.time` 可覆盖 | `[300, 600]` |
| `client` | `str` | 全局客户端名, `emby.account.client` 可覆盖 | `Hills` |
| `device` | `str` | 全局设备名称, `emby.account.device` 可覆盖 | (未配置时随机生成并缓存) |
| `device_id` | `str` | 全局设备 ID, `emby.account.device_id` 可覆盖 | (未配置时随机生成并缓存) |
| `client_version` | `str` | 全局客户端版本, `emby.account.client_version` 可覆盖 | `1.6.1` |
| `useragent` | `str` | 全局用户代理, `emby.account.useragent` 可覆盖 | `Hills/1.6.1 (android; 15)` |

### `emby.account` 子项

该子项用于配置一个或多个 Emby 或 Jellyfin 服务器账户. 与 `[emby]` 中同名的全局设置同时存在时, 账号级设置优先.
...
| `time` | `list` / `int` | 模拟观看的随机时间范围, 设为小于0 (`time = -1`) 以仅登陆不观看 (秒); 默认使用全局 `emby.time` | `[300, 600]` |
...
| `device` | `str` | 自定义的设备名称 (默认使用全局 `emby.device`; 若全局未配置则随机生成并缓存) | |
| `client` | `str` | 自定义的客户端名 (默认使用全局 `emby.client`; 若全局未配置则使用 `Hills`) | |
| `client_version` | `str` | 自定义的客户端版本 (默认使用全局 `emby.client_version`; 若全局未配置则使用 `1.6.1`) | |
| `useragent` | `str` | 自定义的用户代理 (默认使用全局 `emby.useragent`; 若全局未配置则使用 `Hills/1.6.1 (android; 15)`) | |
| `device_id` | `str` | 自定义的设备 ID (默认使用全局 `emby.device_id`; 若全局未配置则随机生成并缓存) | |
...
同时, 默认设置的请求方式是针对于 Hills 调整的, 更改模拟的客户端可能会被识别.
:::
...
您也可以使用 `--emby-account` 按 `name` 选择要运行的 `[[emby.account]]` 条目:

```bash
embykeeper -e --emby-account alpha,beta -i -o
```

`--emby-account` 可以重复使用, 也可以使用逗号分隔多个名称.

`--emby-account` 必须与 `-e` / `--emby` 一起使用.
